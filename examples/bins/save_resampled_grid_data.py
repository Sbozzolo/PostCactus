...
"""This script dumps a specific grid variable resampled to a given grid into a
file."""
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=500,
        help=(
            "Resolution of the resampled data"
            + "(default: %(default)s)"
        ),
    )
    ...
    if len(args.type) != len(args.origin):
        raise ValueError(f"x0 ({args.origin}) and type ({args.type}) are incompatible")
    ...
    logger.debug(f"Reading {iteration}")

    data = var[iteration].to_UniformGridData(shape, x0, x1, iteration=iteration)
